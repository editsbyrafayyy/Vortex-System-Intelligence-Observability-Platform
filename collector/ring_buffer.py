...
class RingBuffer: # ring buffer 
    ...
    def push(self, snapshot: dict) -> None: # main operation, pushing the 
        """
        Write a snapshot into the next slot.
        If the buffer is full, the oldest snapshot is silently overwritten.
        """
        self._slots[self._head] = snapshot ''' important note: python always stores ptrs of objects, never the objects itself, so here
